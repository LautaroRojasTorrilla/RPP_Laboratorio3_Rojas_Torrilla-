...
let people = jsonArray.map((item) => {
  return "ciudad" in item && "publicado" in item
    ? new Heroe(
        item.id,
        item.nombre,
        item.apellido,
...
const addInputByTypePerson = (initialValues) => {
  ...
  const selectedInputs = typeField.value === "heroe" ? inputsAir : inputsLand;
...
const addPerson = (formData) => {
  ...
  const personObject =
  "ciudad" in item && "publicado" in item
  ? new Heroe(
      item.id,
      item.nombre,
      item.apellido,
